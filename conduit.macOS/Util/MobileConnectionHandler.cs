    ...
    class MobileConnectionHandler
    {
        ...
        private async void HandleMimicMessage(dynamic msg)
        {
            ...
            else if (msg[0] == (long) MobileOpcode.Request)
            {
                ...
                var result = await league.RequestAsync(method, path, body);
                var contents = result.Content;
                if (contents.IsNullOrEmpty()) contents = "null";
                DebugLogger.Global.WriteMessage(path + ": " + result.StatusCode);
            ...
            {
                Send("[" + (long) MobileOpcode.VersionResponse + ", \"" + App.VERSION + "\", \"" + Environment.MachineName + "\"]");
            }
        }
